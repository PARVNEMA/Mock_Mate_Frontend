...
export function useGdRoomSocket(params: {
  ...
  const { httpBaseUrl, roomId, userId, enabled, onMessage } = params;
  ...
  const connect = useCallback(() => {
    if (!enabled || !httpBaseUrl || !roomId || !userId) return;
    ...
    ws.onmessage = (event) => {
      try {
        const parsed = JSON.parse(String(event.data)) as GdRoomWsMessage;
        onMessage(parsed);
      } catch {
        // ignore malformed
      }
    ...
    ws.onerror = () => ws.close();
  }, [enabled, httpBaseUrl, roomId, userId, onMessage, send]);
