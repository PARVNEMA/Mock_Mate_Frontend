...
const applyVisemeToMesh = (
  ...
  // Jaw and Mouth Polish for Realism
  const baseJaw =
...
export function Avatar({
  ...
  useFrame((state) => {
    // Natural jitter when speaking to simulate human movement
    const speakingFallback = isSpeaking
      ? ((Math.sin(state.clock.elapsedTime * 12) + 1) / 2) * 0.15
      : 0;

    [headRef, teethRef, beardRef].forEach((ref) => {
      applyVisemeToMesh(
        ref.current,
        activeViseme,
        visemeStrength,
  ...
  return (
    <group {...props} dispose={null}>
      ...
      {/* Static Body Elements (Hair, Clothes, etc) */}
      <skinnedMesh
